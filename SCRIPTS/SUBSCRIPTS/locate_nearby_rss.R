...
if (opt$mode == "V_segments" ) {
  ...
  if (length(overlaps_minus) < 1) {
    ...
    # Fill metadata
    if (length(gff_nhmmer_minus[overlaps_minus@from]) >= 1) gff_nhmmer_minus[overlaps_minus@from]$ID <- paste0("RSS_V-associated_", seq_along(gff_nhmmer_minus[overlaps_minus@from]))
    if (length(gff_nhmmer_minus[which(is.na(gff_nhmmer_minus$ID))]) >= 1) gff_nhmmer_minus[which(is.na(gff_nhmmer_minus$ID))]$ID <- paste("RSS_V-non-associated_", seq_along(gff_nhmmer_minus[which(is.na(gff_nhmmer_minus$ID))]))
