...
opt_list = list(make_option(opt_str = c("-q", "--query"), 
                ...
                make_option(opt_str = c("-s", "--subject"), 
                            ...
                            metavar = "[FILENAME]"),
                make_option(opt_str = c("-t", "--strand"), 
                            action="store",
                            type = "character", 
                            default = NULL, 
                            help = "Exonerate filtered file with only genes", 
...
if (is.null(opt$strand)) {
  print_help(opt_parser)
  stop("A strand must be specified", call.=FALSE)
}
...
# Create ID column
red_gene_anot_subj$ID <- NA

# Add metadata
if (length(red_gene_anot_subj[unique(df_overlaps$subjectHits)]$ID) > 0) {
  red_gene_anot_subj[unique(df_overlaps$subjectHits)]$ID <- paste0("IGHV_gene_", seq_along(unique(df_overlaps$subjectHits)), "_", strand)
}

if (length(red_gene_anot_subj[is.na(red_gene_anot_subj$ID)]$ID) > 0) {
  red_gene_anot_subj[is.na(red_gene_anot_subj$ID)]$ID <- paste0(paste0("IGHV_pseudogene_", seq_along(red_gene_anot_subj[is.na(red_gene_anot_subj$ID)])), "_", strand)
}
...
# Paste
temp_df$renamed <- paste0("IGHV_exon_", renamed_exons_numbers, "_", strand)
...
if (length(red_exon_anot_query[is.na(red_exon_anot_query$ID)]$ID) > 0) {
  red_exon_anot_query[is.na(red_exon_anot_query$ID)]$ID <- "Sequence"
}

# Save as gff3
rtracklayer::export.gff3(red_exon_anot_query, "exon_prediction.gff")